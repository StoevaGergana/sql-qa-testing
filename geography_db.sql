...
--   1. Брой държави във всеки континент

SELECT continent_code, COUNT(*) AS number_countries
FROM countries 
GROUP BY continent_code;
-- или 
SELECT continent_code, COUNT(continent_code) AS number_countries
FROM countries 
GROUP BY continent_code;

--  Средна площ на държавите по континент

SELECT continent_code, AVG(are_in_sq_km) AS avg_area
FROM countries
GROUP BY continent_code;

--  3. Общо население по континенти

SELECT continent_code, SUM(population) AS total_population
FROM countries
GROUP BY continent_code;

 -- Континент с най-много държави
 
 SELECT continents.continent_name, COUNT(continents.continent_code) AS country_count
 FROM continents
 JOIN countries ON countries.continent_code = continents.continent_code
 GROUP BY continents.continent_code
 ORDER BY country_count DESC
 LIMIT 1;
 
USE geography;

 --  Най-висок връх и височина
SELECT peak_name, elevation
FROM peaks
WHERE elevation = (SELECT MAX(elevation) FROM peaks);
...
-- 6. Средна височина на върховете по планинска верига

SELECT mountains.mountain_range, ROUND(AVG(peaks.elevation),2) AS avg_elevation
FROM mountains 
JOIN peaks  ON mountains.id = peaks.mountain_id
GROUP BY mountains.mountain_range
ORDER BY avg_elevation DESC;

--  7. Държави с най-много върхове (чрез връзка planina → durzhava)
 
SELECT countries.country_name AS country, COUNT(peaks.id) AS peak_count
...
SELECT river_name,length
 ...
 -- Средна дължина на реките, които минават през повече от 5 държави
 
SELECT rivers.river_name, rivers.length, COUNT(countries_rivers.country_code) AS country_count
FROM countries_rivers
JOIN rivers ON countries_rivers.river_id = rivers.id
GROUP BY rivers.id
HAVING country_count > 5;

--  Брой държави с повече от 10 милиона души население

SELECT country_name, population AS pop_OVER_10million
FROM countries
WHERE population > 10000000
ORDER BY population DESC;

-- Списък на държави в Европа и Азия с население над 50 милиона

SELECT country_name, population, 'Europe' AS continent
FROM countries
WHERE continent_code = 'EU' AND population > 50000000

UNION

SELECT country_name, population, 'Asia' AS continent
FROM countries
WHERE continent_code = 'AS' AND population > 50000000;

-- Списък на държави, които имат или реки, или планини (без дубли)
